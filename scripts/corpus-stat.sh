...
CMD="cat ../examples/example_input.txt"

F=../examples/example_output.txt
...
# done
# Update hitparade
cat $F | cut -f2 -d'^' | cut -f1 -d'/' | sort -f | uniq -c | sort -gr  > ../rus.hitparade.txt
#
# Update glossed hitparade
# bash hitparade.sh > ../hitparade.txt

# Update unknown list
# cat ../hitparade.txt | grep '/\*[^0-9]' > ../unknown.txt
